...
static struct task_info task_info[] = {
  { .name = "Memory Thread 0", .alloc_size = 64,  .sleep_for = 250 },
  { .name = "Memory Thread 1", .alloc_size = 256, .sleep_for = 1500 },
  { .name = "Memory Thread 2", .alloc_size = 1024, .sleep_for = 2000 },
  { .name = "Memory Thread 3", .alloc_size = 4096, .sleep_for = 3000 },
  { .name = "Memory Thread Fast", .alloc_size = 20, .sleep_for = 1000 },
};
...
void memt_func(void *context)
{
  ...
  while (1) {
    ...
    if (allocated) {
      printf("%s - allocated %d at %d ms\n", ti->name, ti->alloc_size, ticks);
    }
    else {
      printf("%s - FAILED at %d ms\n", ti->name, ti->alloc_size, ticks);
    }
...
void memt_func_fast(void *context)
{
  ...
  while (1) {
    ...
    if (++counter == repeat) {
      ...
      if (allocated) {
        printf("%s - fast allocated %d, %d times at %d ms\n",
               ti->name, ti->alloc_size, repeat, ticks);
      }
      else {
      printf("%s - FAILED at %d ms\n", ti->name, ti->alloc_size, ticks);
      }
